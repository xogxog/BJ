...
goal = [[set() for i in range(N)] for _ in range(N)]
...
for _ in range(M):
    ...
    # 승객 태우고 목적지로 이동!
    goal_num = -1*passengers[near_passenger[0]][near_passenger[1]] # 손님이 가야하는 목표지점의 숫자
    ...
    while move_q :
        ...
        if goal_num in goal[r][c] : # 목적지 도착
            arrived = (r,c,d)
            break
        for dr,dc in drc :
            nr = r+dr
